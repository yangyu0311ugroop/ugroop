...
const useDeviceData = id => {
  ...
  const filteredData = items.reduce((acc, value, index) => {
    if (index === 0) return { [value.browser]: 1 };
    if (!acc[value.browser]) return { ...acc, [value.browser]: 1 };
    acc[value.browser] += 1;
    return acc;
  }, {});
...
export const DevicesViewCard = memo(props => {
  const { data, isLoading } = useDeviceData(props.id);
  let content;
...
DevicesViewCard.propTypes = {
  id: PropTypes.number,
};

export default DevicesViewCard;
