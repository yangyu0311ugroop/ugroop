...
function Line1(props) {
  ...
  const { variant } = props;
  const { intl, id } = props;
  const { line1 } = useSelectorCardData({
    cardId: id,
  });
  const getStrippedOwnProps = () =>
    omit(props, ['resaga', 'line1', 'id', 'classes', 'variant']);
  ...
  const renderTextOnly = () => <P {...getStrippedOwnProps()}>{line1}</P>;
...
export default compose(injectIntl)(Line1);
