...
function PostalCode(props) {
  ...
  const { postalCode } = useSelectorCardData({
    cardId: id,
  });
  const getStrippedOwnProps = () =>
    omit(props, ['resaga', 'postalCode', 'id', 'classes', 'variant']);
  ...
  const renderTextOnly = () => <P {...getStrippedOwnProps()}>{postalCode}</P>;
...
export default compose(injectIntl)(PostalCode);
