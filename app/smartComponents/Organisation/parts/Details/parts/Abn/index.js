...
export class Abn extends PureComponent {
  getInputLabelProps = {
    shrink: true,
  };
  ...
  renderTextOnly = () => {
    const { abn } = this.props;
    return <P {...this.getStrippedOwnProps()}>{abn}</P>;
  };

  renderDefault = () => {
    const { classes, abn } = this.props;
    return (
      <React.Fragment>
